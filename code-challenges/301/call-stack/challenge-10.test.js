...
const count = (target, input) => {
    const concInput = input.reduce((acc, arr) => {
        return acc.concat(arr);
    }, []);

    return concInput.reduce((acc, val) => {
        if (val === target) {
            acc++;
        }
        return acc;
...
const divisibleByFiveTwoToThePower = (input) => {
    const multFives = input.map(array => {
        return array.filter(val => {
            return (typeof(val) == 'number' && !(val % 5));
        });
    });
    return multFives.map(array => {
        return array.map(val => Math.pow(2, val));
    });
};
...
let findMaleAndFemale = (data) => {
    const ladiesNGents = data.filter(char => {
        return (/male/i.test(char.gender));
    });
    const genderedCharNames = ladiesNGents.reduce((acc, char) => {
        return acc.concat([char.name]);
    }, []);
    return genderedCharNames.join(' and ');
}
...
let findShortest = (data) => {
    const shortestChar = data.reduce((acc, char) => {
        if ((char.height - acc.height) < 0) {
          return char;
        }
        return acc;
    });
