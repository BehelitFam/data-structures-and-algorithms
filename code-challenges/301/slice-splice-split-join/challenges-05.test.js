...
const listFoods = (recipe) => {
    let result = [];
    const regex = /[0-9]+\s\b\S+\b\s/g;
    recipe.ingredients.forEach(item => {
      let ingIndex = item.match(regex)[0].length;
      let ingredient = item.slice(ingIndex);
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(item => {
    let ingStrings = item.split(' ');
    let ingredient = ingStrings.slice(2);
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(step => {
    let stepStrings = step.split(' ');
    let actionVerb = stepStrings.shift();
