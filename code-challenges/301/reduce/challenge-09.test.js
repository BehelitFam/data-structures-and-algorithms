...
const returnNames = (arr) => {
  return arr.reduce((acc,curVal) => acc.concat([curVal.name]), []);
};
...
const reversedString = (str) => {
  const arr = str.split('');
  const revArr = arr.reduce((acc,curVal,index,arr) => {
      return acc.concat([arr[arr.length - index - 1]])}, []);
  return revArr.join('');
};
...
const calculateAverage = (arr) => {
    const arrTotals = arr.reduce((acc, curVal) => {
        acc.count++;
        acc.sum += curVal;
        return acc;
    }, {count: 0, sum: 0});
...
const extractChildren = (arr) => {
    const aListers = arr.filter(char => /a/i.test(char.name));
    return aListers.reduce((acc, char) => {
        if (char.children) {
