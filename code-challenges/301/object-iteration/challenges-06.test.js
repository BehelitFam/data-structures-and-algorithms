...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(char => {
    ...
    if(charEntries[0][1].toUpperCase() === character.toUpperCase()) {
        if (charEntries[2][1].length) {
            isParent = true;
        }
    }
  });
...
const houseSize = (arr) => {
  ...
  arr.forEach(char => {
    ...
    sizes.forEach(house => {
        if (house.house.toUpperCase() === char.house.toUpperCase()) {
            house.members += 1 + char.children.length;
            houseExists = true;
