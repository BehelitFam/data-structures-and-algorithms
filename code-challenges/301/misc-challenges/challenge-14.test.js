...
let biggerThanLuke = (arr) => {
    let skyWalkForce = arr.reduce((acc, char) => {
        if (char.name === 'Luke Skywalker') {
            acc = char.mass;
        }
        return acc;
    }, 0);
    let aintNoMoons = arr.filter(character => {
        let charMass = parseInt(character.mass);
        return parseInt(skyWalkForce) < charMass;
    });
    return aintNoMoons.map(character => character.name).join(' - ');
}
...
// Doesn't work for arrays of objects with different numbers of keys.
const sortBy = (property, arr) => {
    let propIndex;
    let keys = Object.keys(arr[0]);
    for (let i = 0; i < keys.length; i++) {
        if(keys[i].toUpperCase() === property.toUpperCase()) {
            propIndex = i;
        }
    }
    return arr.sort((a,b) => {
        return Object.values(a)[propIndex] > Object.values(b)[propIndex];
    });
};
...
const detectTicTacToeWin = (board) => {
    var won = false;
    for (let i = 0; i < 3; i++) {
        if (ticAttack(0, i, 1, i, 2, i, board) || ticAttack(i, 0, i, 1, i, 2, board)) {
            won = true;
        }
    }
    if (ticAttack(0, 0, 1, 1, 2, 2, board) || ticAttack(0, 2, 1, 1, 2, 0, board)) {
        won = true;
    }
    return won;
}

function ticAttack (x1, y1, x2, y2, x3, y3, board) {
    return board[y1][x1] === board[y2][x2] && board[y1][x1] === board[y3][x3] && board[y1][x1]; 
}
