...
const firstLetters = (arr) => {
    return arr.map(stringydoo => stringydoo.charAt(0));
};
...
const findHappiness = (arr) => {
    return arr.filter(stringydoo => /:\)/.test(stringydoo));
};
...
const standardizePhoneNumbers = (arr) => {
    return arr.map(number => {
        ...
        let digits = chars.filter(char => /\d/.test(char));
        return digits.join('');
    });
};
...
const onlyOddChars = (str) => {
    let chars = str.split('');
    let oddChars = chars.filter((char, index) => index % 2);
...
const allHappy = (arr) => {
    const isHappy = arr.map((val) => {
      return /:\)/.test(val);
    });
    return !isHappy.includes(false);
};
...
const findEvery = (arr, target) => {
  const haveTargets = arr.map((val) => {
    return val.includes(target);
  });
  return !haveTargets.includes(false);
};
...
const unenrollBrook = (arr) => {
  return arr.map(roster => {
    return roster.filter(val => !val.includes('Brook'));
  });
};
