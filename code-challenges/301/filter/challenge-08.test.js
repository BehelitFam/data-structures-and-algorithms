...
const getStatName = (arr, minBaseStat) => {
    const statNames = arr.map(val => val.stat.name);
    return statNames.filter((statName, index) => arr[index].baseStat > minBaseStat);
};
...
//     return filteredStats.map(stat => stat.stat.name);
// };
...
const evenOddNumericValues = (arr) => {
    const arrNums = arr.filter(val => typeof(val) === 'number');
    console.log(arrNums);
    return arrNums.map(val => {
        let evenOrOdd = '';
        if ((val%2) === 1) {
            evenOrOdd = 'odd';
        } else if (!(val%2)) {
